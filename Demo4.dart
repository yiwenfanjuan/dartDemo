void main() {
  ...
  testList.add("嘤嘤嘤");
  //替换元素，下面的方法将testList6中的第一个和第二个元素替换为testList中的元素
  ...
  var testList7 = ["test","test"];
  //替换列表中的元素
  //前两个参数表示要替换列表中的元素的位置，如下面的表示将列表中第三个到第十个元素替换为testList5中的元素
  //第二个参数<=列表的长度
  //第一个参数<=第二个参数，狗则会报错
  //通俗的说，就是list会首先删除第一个参数到第二个参数之前的元素，然后将需要替换的list添加在第一个参数的位置。
  print(testList6.length);
...
String check(String str,{String orElse()}){
  if(str == null){
    return orElse();
  }
  return str;
...
var testValue = "哈哈哈";
String checkReduce(String item,String element){
 print("item:  ${item} element:${element}");
  if(item != element){
    item = element;
  }
  print("value:  ${item} element:${element}");
  return item;
}

bool checkLength2(String list){
  print("str===>${list}");
  if(list == null){
    return false;
  }
